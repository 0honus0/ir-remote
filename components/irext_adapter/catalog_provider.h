...
class IrextCatalogProvider {
 public:
  ...
  bool copy_binary(const CatalogCode &entry, std::vector<uint8_t> &binary) const {
    ...
    memcpy_P(binary.data(), irext_catalog::data + entry.offset, entry.length);
    return true;
  }

 protected:
  void mark_ac_brands_(std::vector<bool> &ac_brands) const {
    for (uint16_t code_index = 0; code_index < this->code_count(); code_index++) {
      irext_catalog::ControlCodeEntry entry{};
      memcpy_P(&entry, &irext_catalog::control_codes[code_index], sizeof(entry));
      if (entry.category_id == REMOTE_CATEGORY_AC && entry.brand_index < ac_brands.size())
        ac_brands[entry.brand_index] = true;
      if ((code_index & 0xFFU) == 0) yield();
    }
  }
