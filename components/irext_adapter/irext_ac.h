...
class IrextAc {
 public:
  ...
  bool send(const t_remote_ac_status &status, uint8_t key_code) {
    ...
    // Allocate on demand so the 4 KB USER_DATA buffer is not permanent BSS.
    std::vector<uint16_t> timings(USER_DATA_SIZE);
    t_remote_ac_status mutable_status = status;
    const uint16_t count = ir_decode(key_code, timings.data(), &mutable_status);
    this->close_();
    if (count == 0 || count > USER_DATA_SIZE) {
      this->status_ = "IRext 无法生成该指令";
      return false;
    }
    const bool sent = this->transmitter_.send_raw(timings.data(), count);
